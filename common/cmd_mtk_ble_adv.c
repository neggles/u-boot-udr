...
//STP format
const char stp_head_tx_flow1_cmd[] = {0x80, 0x00, 0x0A, 0x00};
const char stp_head_tx_flow2_cmd[] = {0x80, 0x00, 0x04, 0x00};
const char stp_head_tx_flow3_cmd[] = {0x80, 0x00, 0x13, 0x00};
const char stp_head_tx_flow4_cmd[] = {0x80, 0x00, 0x24, 0x00};
const char stp_head_tx_flow5_cmd[] = {0x80, 0x00, 0x24, 0x00};
const char stp_head_tx_flow6_cmd[] = {0x80, 0x00, 0x05, 0x00};
...
static int get_ble_tx_cmd(u8 idx, u8 tx_cmd_buf[])
{
	...
	switch(idx)
	{
		case BLE_PW_ON_IDX:
			if(STP_ENABLE)
			{
				memcpy(tx_cmd_buf, stp_head_tx_flow1_cmd, STP_HEAD_FORMAT_LEN);
		...
		case BLE_HCI_RESET_IDX:
			if(STP_ENABLE)
			{
				memcpy(tx_cmd_buf, stp_head_tx_flow2_cmd, STP_HEAD_FORMAT_LEN);
				len = STP_HEAD_FORMAT_LEN;
			}
		...
		case BLE_HCI_SET_ADV_PARAM_IDX:
			if(STP_ENABLE)
			{
				memcpy(tx_cmd_buf, stp_head_tx_flow3_cmd, STP_HEAD_FORMAT_LEN);
				len = STP_HEAD_FORMAT_LEN;
			}
		...
		case BLE_HCI_SET_ADV_DATA_IDX:
			if(STP_ENABLE)
			{
				memcpy(tx_cmd_buf, stp_head_tx_flow4_cmd, STP_HEAD_FORMAT_LEN);
				len = STP_HEAD_FORMAT_LEN;
			}
		...
		case BLE_HCI_SET_SCAN_RESP_DATA_IDX:
			if(STP_ENABLE)
			{
				memcpy(tx_cmd_buf, stp_head_tx_flow5_cmd, STP_HEAD_FORMAT_LEN);
				len = STP_HEAD_FORMAT_LEN;
			}
		...
		case BLE_HCI_SET_ADV_ENABLE_IDX:
			if(STP_ENABLE)
			{
				memcpy(tx_cmd_buf, stp_head_tx_flow6_cmd, STP_HEAD_FORMAT_LEN);
				len = STP_HEAD_FORMAT_LEN;
			}
...
static int do_mtk_ble_adv(cmd_tbl_t *cmdtp, int flag, int argc,	char *const argv[])
{
	...
	value = DRV_Reg32(0x1A143510);
	value = value | (0x1 << 8);
	DRV_WriteReg32(0x1A143510, value);
	value = DRV_Reg32(0x1A143510);
	printf("0x1A143510 value: %x\n", value);
	mdelay(100);
...
/* this is used to init the mt7622 board gpio */
static int do_mtk_ble_init(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	printf("=========================UART_3 INIT=====================\n");
	mtk_2nd_serial_console()->start();
	mtk_2nd_serial_console()->setbrg();
...
U_BOOT_CMD(mtk_ble_init, 1, 0, do_mtk_ble_init,
			"Init BLE",
			"this feature is based on uart3 function enable\n"
);
