...
#define CONFIG_ROOTARGS_EMMC_DUAL_0 \
	"root=/dev/mmcblk0p2 rootfstype=squashfs rootwait"
#define CONFIG_ROOTARGS_EMMC_DUAL_1 \
	"root=/dev/mmcblk0p3 rootfstype=squashfs rootwait"
#define CONFIG_BOOTARGS_COMMON	\
	"console=ttyS0,115200 panic=3"

#define CONFIG_BOOTCMD_EMMCDUAL_BOOTARGS \
	"rootargsemmcdual0="						\
		"setenv rootargs " CONFIG_ROOTARGS_EMMC_DUAL_0 "\0"	\
	"rootargsemmcdual1="						\
		"setenv rootargs " CONFIG_ROOTARGS_EMMC_DUAL_1 "\0"	\
	"bootargsemmcdual0="							\
		"run rootargsemmcdual0; setenv bootargs $rootargs " CONFIG_BOOTARGS_COMMON " $bootargsextra; printenv bootargs\0"	\
	"bootargsemmcdual1="							\
		"run rootargsemmcdual1; setenv bootargs $rootargs " CONFIG_BOOTARGS_COMMON " $bootargsextra; printenv bootargs\0"	\


#define CONFIG_FIT_CONF \
	"fitbootconf="__stringify(CONFIG_SYS_LOAD_ADDR)"#udmlite@1\0"

/* TODO : mmc init has to be called twice, the first call fails for some reason */
#define CONFIG_BOOTCMD_EMMCDUAL_FIT 	\
	"emmcdualfit="						\
		"mmc device 0;" \
		"mmc init; mmc init;"						\
		"ext4load mmc 0:1 $loadaddr select-image-1;"	\
		"if test $? -eq 0; then run bootargsemmcdual1; ext4load mmc 0:1 $loadaddr uImage.1; else run bootargsemmcdual0; ext4load mmc 0:1 $loadaddr uImage.0; fi;"	\
		"bootm $fitbootconf;" \
...
#define CONFIG_BOOTCMD_EMMCDUAL_FIT_RECOVERY 	\
	"emmcdualfitrecovery=" 					\
		"mmc device 0;" \
		"mmc init; mmc init;"						\
		"ext4load mmc 0:5 $loadaddr uImage.r;"	\
		"run bootargsrecovery;"	\
		"bootm $fitbootconf;" \
		"echo \"Recovery boot failed! Trying TFTP network boot...\";" \
		"setenv serverip 192.168.1.2;" \
		"setenv ipaddr 192.168.1.30;" \
		"tftpboot $loadaddr UDM-mt7622-recovery.bin;" \
		"bootm $fitbootconf;" \
		"reset \0"	\
	""
