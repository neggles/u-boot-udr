...
int der_decode_integer(const unsigned char *in, unsigned long inlen, void *num)
{
   ...
   if (inlen < (1 + 1 + 1)) {
      printf("%s: invalid pkt -2, inlen=%d \n", __func__, inlen);
   ...
   x = 0;
   if ((in[x++] & 0x1F) != 0x02) {
      printf("%s: invalid pkt -1, x=%d, in=%x, in[x-1]=%x in[x]=%x\n", __func__, x-1, in, in[x-1], in[x]);
      return CRYPT_INVALID_PACKET;
   }
   ...
   if ((z & 0x80) == 0x00) {
      ...
      /* will it overflow? */
      if (x + z > inlen) {
         printf("%s: invalid pkt 0, x=%d, z=%d, inlen=%d \n", __func__, x, z, inlen);
         return CRYPT_INVALID_PACKET;
      }
      ...
      if (((x + z) > inlen) || (z > 4) || (z == 0)) {
         printf("%s: invalid pkt, x=%d, z=%d, inlen=%d \n", __func__, x, z, inlen);
         return CRYPT_INVALID_PACKET;
      }
      ...
      if ((x + y) > inlen) {
         printf("%s: invalid pkt 1, x=%d, y=%d, inlen=%d \n", __func__, x, y, inlen);
         return CRYPT_INVALID_PACKET;
      }
